...
class GoogleOTA(commands.Cog):
    ...
    async def gota(self, ctx, fingerprint):
        ...
        checkin_request.checkin.build.device = fingerprint.split('/')[2].split(
            ':'
        )[0]
        ...
        checkin_response = (
            checkin_generator_pb2.AndroidCheckinResponse.FromString(
                resp.content
            )
        )
        setting = {
            entry.name: entry.value for entry in checkin_response.setting
        }
        ...
        if update_description and update_title and update_url:
            embed = discord.Embed(
                title=update_title.decode(),
                description=update_description.decode()[:4096],
            )
