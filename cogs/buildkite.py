...
class Buildkite(commands.Cog):
    ...
    async def build(
        self, ctx, device: str, version: str, release_type: str = "nightly", *args
    ):
        tags = version.split(",")
        if "#" in tags[0]:
        ...
        resp = requests.post(
            ...
            json=data,
            headers={"Authorization": f"Bearer {os.environ.get('BUILDKITE_TOKEN')}"},
        )
    ...
    async def rebuild(self, ctx, id: str, *args):
        resp = requests.put(
            f"https://api.buildkite.com/v2/organizations/lineageos/pipelines/android/builds/{id}/rebuild",
            headers={"Authorization": f"Bearer {os.environ.get('BUILDKITE_TOKEN')}"},
        )
    ...
    async def cancel(self, ctx, id: str, *args):
        resp = requests.put(
            f"https://api.buildkite.com/v2/organizations/lineageos/pipelines/android/builds/{id}/cancel",
            headers={"Authorization": f"Bearer {os.environ.get('BUILDKITE_TOKEN')}"},
        )
        if resp.status_code == 200:
    ...
    @commands.has_role("Project Director")
    @commands.command(
        name="crowdin", help="start crowdin build for a branch. example: lineage-20.0"
    )
